...
class Asteroid:
    """Asteroida z proceduralnym kształtem wielokątnym.

    Zadanie **: konstruktor przyjmuje rozmiar jako string ('large'/'medium'/'small').
    Promień, prędkość i liczba wierzchołków wyznaczane automatycznie.
    """
    ...
    def __init__(self, x, y, size="large"):
        ...
        # Proceduralne wierzchołki w układzie lokalnym (Zadanie 5)
        self.verts = []
    ...
    def _draw_at(self, cx, cy):
        """Rysuje obrócony wielokąt w punkcie (cx, cy)."""
        n = len(self.verts)
        for i in range(n):
...
def spawn_asteroids(count):
    """Generuje listę asteroid z losowymi pozycjami i rozmiarami."""
    asteroids = []
